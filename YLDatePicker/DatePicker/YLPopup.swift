...
class YLPopup: NSObject {
    ...
    // 内容视图高度
    public var contentHeight = 216 + (SCREEN_HEIGHT >= 812 ? 34.0 : 0.0)
    ...
    private func barButtonItemWith(title: String?, action: Selector?,  adjustment: UIOffset = .zero) -> UIBarButtonItem! {
        ...
        item.setTitleTextAttributes([NSAttributedString.Key.font: UIFont.boldSystemFont(ofSize: 17)], for: .normal)
        return item
    }
}
